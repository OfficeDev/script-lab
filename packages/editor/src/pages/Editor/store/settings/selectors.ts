...
export const getMonacoTheme = (state: IState): "vs" | "vs-dark" | "hc-black" => {
  ...
  }[get(state)["editor.theme"]] as "vs" | "vs-dark" | "hc-black";
};
...
export const getIsPrettierEnabled = (state: IState): boolean => get(state)["editor.prettier"];
export const getIsAutoFormatEnabled = (state: IState): boolean =>
  get(state)["editor.prettier.autoFormat"];

export const getMonacoOptions = (state: IState): monaco.editor.IEditorConstructionOptions => {
